...
Hay un total de `{python} int(len(renabap_pba_intersect))` barrios populares en el Partido de La Plata, que representan `{python} int(renabap_pba_intersect['familias_aproximadas'].sum())` familias. Estos barrios ocupan `{python} int(renabap_total_area_ha)` hectáreas del Partido de La Plata, o `{python} f"{percentage_coverage:.1f}"` por ciento del partido.
