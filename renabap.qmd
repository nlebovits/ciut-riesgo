...
- [📥 Click aquí para descargar los datos de exposición de edificios a nivel de barrio](https://arg-fulbright-data.s3.us-east-2.amazonaws.com/renabap/exposicion-barrio-la-plata-03-09-2025.csv)
- [📥 Click aquí para descargar los datos de exposición de edificios a nivel de cuenca y eje](https://arg-fulbright-data.s3.us-east-2.amazonaws.com/renabap/exposicion-cuenca-la-plata-03-09-2025.csv)
...
# Obtener todas las edificiones que intersectan con los barrios (corregir warning de deprecación)
...
El territorio total del Partido de La Plata incluye `{python} format_number(int(peligro_alta_ha))` hectáreas de peligro alto (`{python} peligro_alta_pct`% del partido) y `{python} format_number(int(peligro_media_ha))` hectáreas de peligro medio (`{python} peligro_media_pct`% del partido). Respecto a la exposición de los barrios populares a peligros de inundación, se encontró que `{python} int(barrios_total_peligro)` barrios (`{python} float(round((barrios_total_peligro / total_barrios) * 100, 1))`% del total) intersectan con zonas de peligro. Específicamente, `{python} barrios_solo_alta` barrios intersectan únicamente con zonas de peligro alto, `{python} barrios_solo_media` barrios intersectan únicamente con zonas de peligro medio, y `{python} barrios_ambos_peligros` barrios presentan tanto peligro alto como medio dentro de sus límites.
...
# | fig-cap: "Ejemplo de discrepancia en los datos del RENABAP: el barrio Los Pinos con límites oficiales y edificiones detectadas"
...
if len(barrio_5688) == 0:
    ...
    fig.suptitle(
        f"Barrio {barrio_nombre} - Límites y edificiones",
        fontsize=16,
        fontweight="bold",
        y=0.98,
    )
...
# | fig-subcap:
# |   - "Cuencas por edificiones expuestas"
# |   - "Ejes por edificiones expuestas"
...
En los barrios populares de La Plata se identifican `{python} format_number(total_buildings_exposed)` edificiones expuestas a peligros de inundación, lo que representa el `{python} percentage_exposed`% del total de edificiones en asentamientos informales. De estas, `{python} format_number(total_buildings_high_hazard)` edificiones se encuentran en zonas de peligro alto y `{python} format_number(total_buildings_medium_hazard)` en zonas de peligro medio. La exposición se concentra principalmente en los alrededores del casco urbano de La Plata, con una distribución característica donde pocos barrios presentan exposición muy alta, seguidos por un grupo de exposición media. `{python} top_5_barrios_alta.iloc[0]['nombre_barrio']` lidera con `{python} int(top_5_barrios_alta.iloc[0]['edificios_expuestos'])` edificiones expuestas a peligro alto (`{python} float(top_5_barrios_alta.iloc[0]['porcentaje'])`% del barrio), seguido por `{python} top_5_barrios_alta.iloc[1]['nombre_barrio']` con `{python} int(top_5_barrios_alta.iloc[1]['edificios_expuestos'])` edificiones (`{python} float(top_5_barrios_alta.iloc[1]['porcentaje'])`%), `{python} top_5_barrios_alta.iloc[2]['nombre_barrio']` con `{python} int(top_5_barrios_alta.iloc[2]['edificios_expuestos'])` (`{python} float(top_5_barrios_alta.iloc[2]['porcentaje'])`%), `{python} top_5_barrios_alta.iloc[3]['nombre_barrio']` con `{python} int(top_5_barrios_alta.iloc[3]['edificios_expuestos'])` (`{python} float(top_5_barrios_alta.iloc[3]['porcentaje'])`%), y `{python} top_5_barrios_alta.iloc[4]['nombre_barrio']` con `{python} int(top_5_barrios_alta.iloc[4]['edificios_expuestos'])` edificiones (`{python} float(top_5_barrios_alta.iloc[4]['porcentaje'])`%). A nivel de cuencas hidrográficas, `{python} top_5_cuencas.iloc[0]['Cuenca']` concentra la mayor exposición con `{python} int(top_5_cuencas.iloc[0]['edificios_expuestos_total'])` edificiones expuestas (`{python} int(top_5_cuencas.iloc[0]['edificios_expuestos_alta'])` a peligro alto y `{python} int(top_5_cuencas.iloc[0]['edificios_expuestos_media'])` a peligro medio), principalmente debido a la presencia de `{python} top_5_barrios_alta.iloc[0]['nombre_barrio']` y otros asentamientos importantes. Le sigue `{python} top_5_cuencas.iloc[1]['Cuenca']` con `{python} int(top_5_cuencas.iloc[1]['edificios_expuestos_total'])` edificiones (`{python} int(top_5_cuencas.iloc[1]['edificios_expuestos_alta'])` alta, `{python} int(top_5_cuencas.iloc[1]['edificios_expuestos_media'])` media) y `{python} top_5_cuencas.iloc[2]['Cuenca']` con `{python} int(top_5_cuencas.iloc[2]['edificios_expuestos_total'])` edificiones (`{python} int(top_5_cuencas.iloc[2]['edificios_expuestos_alta'])` alta, `{python} int(top_5_cuencas.iloc[2]['edificios_expuestos_media'])` media). Esta concentración refleja tanto la densidad de asentamientos informales como su ubicación en zonas de alto riesgo hidrológico.
